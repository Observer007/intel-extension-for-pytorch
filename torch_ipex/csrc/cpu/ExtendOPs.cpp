...
#include "template/interaction.hpp"
...
#pragma omp simd
  ...
  inline at::Tensor _interaction_forward(const std::vector<at::Tensor> &input)
  {
    ...
    auto interaction = sc::gen_interaction_fwd("interaction", {batch_size, vector_size},
                                               {batch_size, vector_size}, input.size() - 1, sc::datatypes::f32);
    auto f = interaction.generate();
    auto fptr = sc::cfake_jit().get_entry_func(f, true);
    std::vector<sc::generic_val> generic_args;
    generic_args.reserve(input.size() + 1);
    ...
    generic_args.emplace_back(out_data);
    for (auto i = 0; i < input.size(); i++)
    {
      generic_args.emplace_back(input_data[i]);
    }
    fptr->call_generic(generic_args.data());
    return out;
  }
  ...
  inline std::vector<at::Tensor> _interaction_backward(const at::Tensor &grad_out, const std::vector<at::Tensor> &input)
  {
    ...
    int64_t batch_size = input[0].sizes()[0];
    uint32_t vector_size = input[0].sizes()[1];
    uint32_t out_size = grad_out[0].sizes()[1];
    std::vector<uint32_t> feature_sizes(input.size());
    std::vector<at::Tensor> output(input.size());
    std::vector<T *> input_data(input.size());
    std::vector<T *> output_data(input.size());
    auto interaction = sc::gen_interaction_bwd("interaction", {batch_size, out_size}, {batch_size, vector_size},
                                               {batch_size, vector_size}, input.size() - 1, sc::datatypes::f32);
    auto f = interaction.generate();
    auto fptr = sc::cfake_jit().get_entry_func(f, true);
    std::vector<sc::generic_val> generic_args;
    generic_args.reserve(2 * input.size() + 1);
    auto grad_out_data = grad_out.data_ptr<T>();
    generic_args.emplace_back((void *)grad_out_data);
    for (int i = 0; i < input.size(); i++)
    {
    ...
    for (int i = 0; i < input.size(); i++)
    {
      generic_args.emplace_back((void *)output_data[i]);
    }
    for (int i = 0; i < input.size(); i++)
    {
      generic_args.emplace_back((void *)input_data[i]);
    }
    fptr->call_generic(&generic_args[0]);
    return output;
  }
